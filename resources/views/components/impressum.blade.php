{{-- Impressum - zentral gepflegt in immowert/legal. Nicht pro Portal aendern:
     Anpassungen gehoeren in config/immowert-legal.php des Pakets. --}}
@php($c = (array) config('immowert-legal.company'))
@php($site = (array) config('immowert-legal.site'))

<h2>Angaben gemäß § 5 DDG</h2>
<x-legal::company-block />

<p>
    Handelsregister: {{ $c['register'] }}<br>
    Registergericht: {{ $c['register_court'] }}
</p>

<p>
    <strong>Vertreten durch:</strong><br>
    {{ implode(', ', (array) ($c['represented_by'] ?? [])) }}
</p>

<h2>Kontakt</h2>
<x-legal::contact-block />

<h2>Umsatzsteuer-ID</h2>
<p>Umsatzsteuer-Identifikationsnummer gemäß § 27a Umsatzsteuergesetz: {{ $c['vat_id'] }}</p>

<h2>Verantwortlich für den Inhalt nach § 18 Abs. 2 MStV</h2>
<x-legal::company-block />

@if (!empty($site['disclaimer']))
    <h2>Verhältnis zu Behörden</h2>
    <p>{{ $site['disclaimer'] }}</p>
@endif
